...
atribuir a essa mesma \textit{label} um $v_i=1$.
      \end{enumerate}
    \item 1º Passo - Atualização de $v_i$ e propagação da $label$.
    \begin{enumerate}
      \item Caso haja mensagens, atualizar o valor de $v_i$.
...
e para a sua $label$ atual. Ver a $label$ que maximiza e em caso 
...
mensagens, querendo dizer que é representante de uma comunidade. 
    \begin{enumerate}    
      \item Verificar o estado do agregador, se tiver informação que todos os 
vértices não mudaram de comunidade então para-se a computação e termina o 
algoritmo. 
      \item Verificar que não recebeu uma mensagem proveniente de um vértice à 
qual se juntou para evitar ciclos. Caso se tenha juntado então escolher a 
$label$ menor entre a que tem atualmente e a que recebeu.
      \item Para todas as mensagem, calcular $v_i$ e enviar o valor atualizado 
...
representante se juntou e caso a $label$ da comunidade do representante 
seja menor do que a $label$ do vértice que mandou a mensagem.
    \end{enumerate}
    \item Repetir a partir do 1º passo.  
    \end{enumerate}
  \end{algorithm}
  
Para possíveis otimizações, é possível mudar o representante no 3º passo
para que este faça parte da comunidade de modo a que a troca de mensagens pela 
rede seja diminuída, caso sejam utilizados os particionamentos adequados.
...
de agregação para saber o $v_i$ de uma determinada $label_i$,de modo a que o 
valor de $v_i$ não esteja desatualizado nas extremidades da comunidade, daí 
haver a necessidade de cada vértice enviar para o representante da sua 
comunidade.
...
atribuir a essa mesma \textit{label} um $v_i=1$.
    \item Caso haja mensagens, atualizar o valor de $v_i$. Enviar para os seus 
vértices adjacentes uma mensagem com vértice de origem, \textit{label} do 
vértice de origem e valor de $v_i$ da \textit{label}.
     \item Calcular a formula \ref{apmeq} para as \textit{labels} que recebeu e 
para a sua \textit{label} atual. Ver a \textit{label} que maximiza e em caso 
caso de empate, escolher a \textit{label} menor. Enviar para o representante da 
sua comunidade uma mensagem. Num agregador, agregar o estado se o vértice mudou 
ou não de comunidade. Fazer \textit{halt} ao vértice.
    \item Caso esteja ativo neste passo então recebeu mensagens, querendo dizer 
que é representante de uma comunidade. Verificar o estado do agregador, se 
tiver informação que todos os vértices não mudaram de comunidade então para-se 
a computação e termina o algoritmo. Verificar que não recebeu uma mensagem 
proveniente de um vértice à qual se juntou para evitar ciclos. Caso se tenha 
juntado então escolher a \textit{label} menor entre a que tem atualmente e a 
que recebeu. Para todas as mensagem, calcular $vi$ e enviar o valor atualizado 
para todos os vértices que lhe enviaram a mensagem. No cálculo do $vi$ há que 
ter em conta a necessidade de ignorar a mensagem de um vértice que o 
representante se juntou e caso a \textit{label} da comunidade do.
    \item Repetir a partir do passo 2.  
    \end{enumerate}
  \end{algorithm}

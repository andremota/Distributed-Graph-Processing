...
O algoritmo apresentado anteriormente pode ser usado em grafos de grandes dimensões em plataformas distribuídas. Contudo, existe algumas mudanças quanto à várias fazes do algoritmo. 
  ...
  \begin{algorithm}
    ...
    \begin{enumerate}
      \item Começar por atribuir uma \textit{label} única a cada vértice e atribuir a essa mesma \textit{label} um $v_i=1$. Enviar para os vértices adjacentes o pare \{$label_i$,$v_i$\}.
      \item Agregar todos os pares \{$label_i$, $v_i$\} que foram recebidos e actualizar as entradas de $v_i$ para cada $label_i$ correspondente.
      \item Calcular a formula \ref{apmeq} para as \textit{labels} que recebeu e para a sua \textit{label} actual independentemente se a recebeu ou não.Ver a \textit{label} que maximiza. Caso a \textit{label} mude, enviar para os seus adjacentes o novo valor de $v_i$ correspondente à sua \textit{label} actual.
    \end{enumerate}
  \end{algorithm}

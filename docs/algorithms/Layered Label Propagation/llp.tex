...
de desempate tem de se manter constante para todo o algoritmo.
      \item Se a iteração for par\footnote{Uma iteração, par ou 
ímpar, é dada pela seguinte expressão: (nº superstep/3) mod 3, em que 0 
indica que é uma iteração par e 1 uma iteração ímpar.}, não permitir que 
vértices vão para comunidades inferiores à sua.
...
termina o algoritmo. 
      ...
      \begin{enumerate}
	\item Calcular $v_i$.
	\item Verificar que não recebeu uma mensagem 
proveniente de um vértice à qual se juntou para evitar ciclos. Caso se tenha 
juntado então escolher a $label$ menor entre a que tem atualmente e a que 
recebeu.
	\item No cálculo do $v_i$ há que ter em conta a 
necessidade de ignorar a mensagem de um vértice que o representante se juntou e 
caso a $label$ da comunidade do representante seja menor do que a $label$ do 
vértice que mandou a mensagem.
      \end{enumerate}
